...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("\nUploading a file:");
    let uploaded_file = client.upload_file(temp_file_path, FilePurpose::FineTune).await?;
    println!(
    ...
    let uploaded_bytes = client.upload_bytes("bytes_example.jsonl", content, FilePurpose::FineTune).await?;
    println!(
